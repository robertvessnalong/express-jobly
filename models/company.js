...
class Company {
  ...
  static async filter(queryData) {
    const validator = jsonschema.validate(queryData, companyFilter);
    if (!validator.valid) {
    ...
    const selector = name ? 'AND' : 'WHERE';
    if (minEmployees > maxEmployees) {
      throw new BadRequestError('Please make sure min is lower than max', 400);
    } else if (minEmployees && maxEmployees) {
      queryString += `${selector} num_employees BETWEEN ${minEmployees} AND ${maxEmployees}`;
    } else if (minEmployees) {
      queryString += `${selector}  num_employees >= ${minEmployees}`;

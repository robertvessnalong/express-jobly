...
class User {
  ...
  static async get(username) {
    ...
    user.applications = userJobs.rows.map((a) => a.job_id);
    return user;
  }
  ...
  /** Apply To Job. Update DB.
   *
   *  Param = Username, JobId
   *  Username = Username of Person Applying
   *  JobId = Job ID
   */

  static async apply(username, jobId) {
    const jobInDB = await db.query(
      `SELECT id
           FROM jobs
           WHERE id = $1`,
      [jobId]
    );
    const job = jobInDB.rows[0];
